...
layout.dbu = 0.001
top = layout.create_cell("oa_sar8_acore5")
...
top.insert(pya.DCellInstArray(bst.cell_index(), pya.DTrans(pya.DVector(256.0, -17.8))))
...
rect("m1", 2.6, SWY - 2.2, 358.5, SWY - 1.6)                  # VSS (+bstrap merge)
rect("m1", 2.6, SWY + 12.4, 359.0, SWY + 13.0)                # VDD (+bstrap feed)

# ---- trk / trkb / vin buses (M3, cell-frame pass-through; vin breaks before inv)
rect("m3", 5.0, TRK_Y - 0.2, 253.3, TRK_Y + 0.2)              # trk: cells + TG + TG2 tap
rect("m3", 5.0, TRKB_Y - 0.2, 355.4, TRKB_Y + 0.2)            # trkb: cells+TG+bstrap ckb
rect("m3", 5.0, VIN_Y - 0.2, 224.1, VIN_Y + 0.2)              # vinr: bits (fed by bstrap)
# ---- gb bus (boosted gate) + jog to the bstrap gb pin
rect("m3", 5.0, -11.08, 254.9, -10.68)
rect("m3", 254.3, -14.3, 256.2, -13.9)
via2(254.5, -14.1)
vwire(254.5, -14.1, -10.88)
via2(254.5, -10.88)
# ---- bstrap out -> vinr: south band under segment B -> riser at the b7 gap
rect("m3", 194.8, -19.35, 256.2, -18.95)
via2(195.0, -19.15)
vwire(195.0, -19.15, -13.3)
via2(195.0, -13.3)
# ---- acore-level vin pin: top-level stub onto the bstrap vin band (east,
# clear of the gb riser pad at 254.5)
rect("m3", 255.7, -11.1, 259.0, -10.7)
top.shapes(L["m3p"]).insert(pya.DBox(258.4, -11.1, 259.0, -10.7))
top.shapes(L["m3l"]).insert(pya.DText("vin", pya.DTrans(pya.DVector(258.7, -10.9))))
# ---- bstrap VDD feed (west of its boot cap)
via1(291.6, SWY + 12.7)
vwire(291.6, -10.1, SWY + 12.7)
via1(291.6, -10.1)
...
vwire(255.5, -13.3, VIN_Y)
via2(255.5, VIN_Y)
rect("m3", 251.5, VIN_Y - 0.2, 255.7, VIN_Y + 0.2)
top.shapes(L["m3p"]).insert(pya.DBox(253.5, -10.02, 254.0, -9.62))
top.shapes(L["m3l"]).insert(pya.DText("vcm", pya.DTrans(pya.DVector(253.75, -9.82))))
...
# ================= acore5: symmetric replica switch bank =================
NETS2 = ["b7r", "b6r", "b5r", "b4r", "b3r", "b2r", "b1r", "b0r", "termr"]
RAIL_B = {n: 176.0 + 2.2 * i for i, n in enumerate(NETS2)}
JOG_Y = {n: y for n, y in zip(NETS2, (-18.2, -17.4, -16.6, -15.8, -9.3, -8.5, -7.7, -6.9, -6.1))}
COL_X = {n: x for n, x in zip(NETS2, (194.6, 195.25, 195.9, 196.55, 199.05, 199.7, 200.35, 201.0, 201.65))}
SWY2 = -52.0
SW_X2 = {n: 215.0 + 13.8 * i for i, n in enumerate(NETS2)}
BAND2 = {n: -55.0 - 0.9 * i for i, n in enumerate(NETS2)}
for n in NETS2:
    top.insert(pya.DCellInstArray(sw10.cell_index(), pya.DTrans(pya.DVector(SW_X2[n], SWY2))))
tg2x = 340.0
top.insert(pya.DCellInstArray(tg.cell_index(), pya.DTrans(pya.DVector(tg2x, SWY2))))
# row2 rails + feeds from row1 (east of bstrap)
rect("m1", 213.0, SWY2 - 2.2, 358.5, SWY2 - 1.6)
rect("m1", 212.2, SWY2 + 12.4, 358.5, SWY2 + 13.0)
via1(357.0, SWY - 1.9)
vwire(357.0, SWY2 - 1.9, SWY - 1.9)
via1(357.0, SWY2 - 1.9)
via1(358.0, SWY + 12.7)
vwire(358.0, SWY2 + 12.7, SWY + 12.7)
via1(358.0, SWY2 + 12.7)
# per-bit path v2: rail pad -> (3 west bits: M3 dogleg past the ctl drops)
# -> M2 vertical with x-monotone depth (inverted ladder: no band piercing)
# -> deep M3 band (-26.75-0.82k) -> row2 cell bot riser
...
for n in NETS2:
    ...
    via2(xb2, yd)
    vwire(xb2, yd, SWY2 + 5.5)
    via2(xb2, SWY2 + 5.5)
# static midcode ties: b7r ctl -> VSS2 rail, others -> VDD2 rail
for i, n in enumerate(NETS2):
    xt2 = SW_X2[n] - 0.7
    via2(xt2, SWY2 + 3.9)
    if i == 0:
        vwire(xt2, SWY2 - 1.9, SWY2 + 3.9)
        via1(xt2, SWY2 - 1.9)
    else:
        vwire(xt2, SWY2 + 3.9, SWY2 + 12.7)
        via1(xt2, SWY2 + 12.7)
# dead gates: gb2 bus -> VSS2 ; trkb2 bus -> VDD2 (both switches off = loading dummies)
rect("m3", 213.2, SWY2 + 6.92, 326.2, SWY2 + 7.32)
via2(213.4, SWY2 + 7.12)
vwire(213.4, SWY2 + 7.12, SWY2 - 1.9)
via1(213.4, SWY2 - 1.9)
rect("m3", 212.4, SWY2 + 2.9, 326.2, SWY2 + 3.3)
via2(212.6, SWY2 + 3.1)
vwire(212.6, SWY2 + 3.1, SWY2 + 12.7)
via1(212.6, SWY2 + 12.7)
# vcm: row2 vin band + east riser over to the row1 TG vcm feed
rect("m3", 214.4, SWY2 + 4.5, 359.0, SWY2 + 4.9)
via2(358.8, SWY2 + 4.7)
vwire(358.8, SWY2 + 4.7, -9.82)
via2(358.8, -9.82)
rect("m3", 253.5, -10.02, 359.0, -9.62)
via2(253.7, -9.82)
vwire(253.7, -13.3, -9.82)
via2(253.7, -13.3)
rect("m3", 251.5, -13.5, 253.9, -13.1)
# replica top TG: bot -> topr via east TM1 corridor; vin -> vcm band; gates trk/trkb
rect("m3", tg2x + 12.1, SWY2 + 5.3, 353.4, SWY2 + 5.7)
via2(353.2, SWY2 + 5.5)
vwire(353.2, SWY2 + 5.5, 114.9)
place_vs(VS_M2_TM1, 353.2, 114.9)
rect("tm1", 294.0, 113.9, 354.4, 115.9)
rect("m3", tg2x - 1.0, SWY2 + 4.5, tg2x + 0.4, SWY2 + 4.9)
# trk south route: riser 253.0 down -> band -21.5 -> riser 344.2 -> TG2 trk band
via2(253.0, TRK_Y)
vwire(253.0, -21.5, TRK_Y)
via2(253.0, -21.5)
rect("m3", 252.8, -21.7, 354.6, -21.3)
via2(354.4, -21.5)
vwire(354.4, -45.7, -21.5)
via2(354.4, SWY2 + 6.3)
rect("m3", tg2x + 12.1, SWY2 + 6.1, 354.6, SWY2 + 6.5)
# trkb: riser 343.0 from the extended trkb bus down to TG2 trkb band
via2(355.2, TRKB_Y)
vwire(355.2, SWY2 + 3.1, TRKB_Y)
via2(355.2, SWY2 + 3.1)
rect("m3", tg2x + 12.1, SWY2 + 2.9, 355.4, SWY2 + 3.3)

layout.write("oa_sar8_acore5.gds")
b = top.dbbox()
print(f"acore5: {b.width():.0f} x {b.height():.0f} um = {b.width()*b.height()/1e6:.3f} mm2")
