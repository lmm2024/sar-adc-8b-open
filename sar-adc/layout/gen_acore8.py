...
layout.dbu = 0.001
top = layout.create_cell("oa_sar8_acore8")
...
Y2 = -50.0
SW_XB = {"inv": 150.4, "tgc": 164.2, "tg2": 178.0}
BSX = 99.6
...
rect("m1", 96.4, Y2 - 2.2, 191.0, Y2 - 1.6)                    # VSS2 (bstrap + island)
rect("m1", 149.2, Y2 + 12.4, 191.0, Y2 + 13.0)                 # VDD2 (island only; bstrap has its own rail)
via1(96.8, SWY - 1.9)
vwire(96.8, Y2 - 1.9, SWY - 1.9)
via1(96.8, Y2 - 1.9)
via1(150.0, SWY + 12.7)
vwire(150.0, Y2 + 12.7, SWY + 12.7)
via1(150.0, Y2 + 12.7)
...
via2(117.7, -35.6)
rect("m3", 98.2, -35.8, 117.9, -35.4)                          # gb shelf
via2(98.4, -35.6)
vwire(98.4, Y2 + 3.9, -35.6)
via2(98.4, Y2 + 3.9)
rect("m3", 98.2, Y2 + 3.7, 99.8, Y2 + 4.1)                     # -> bstrap g pin (west end)

# ---- bstrap out -> vinr: south band under segment B -> riser at the b7 gap
via2(116.9, -13.3)
vwire(116.9, -37.4, -13.3)
via2(116.9, -37.4)
rect("m3", 97.4, -37.6, 117.1, -37.2)                          # out shelf
via2(97.6, -37.4)
vwire(97.6, Y2 - 1.0, -37.4)
via2(97.6, Y2 - 1.0)
rect("m3", 97.4, Y2 - 1.2, 99.8, Y2 - 0.8)                     # -> bstrap out pin (west end)

# ---- acore-level vin pin: top-level stub onto the bstrap vin band (east,
# clear of the gb riser pad at 254.5)
rect("m3", 96.5, Y2 + 6.9, 99.6, Y2 + 7.3)
top.shapes(L["m3p"]).insert(pya.DBox(96.5, Y2 + 6.9, 97.1, Y2 + 7.3))
top.shapes(L["m3l"]).insert(pya.DText("vin", pya.DTrans(pya.DVector(96.8, Y2 + 7.1))))
# ---- bstrap VDD feed (west of its boot cap)
via1(147.9, SWY + 12.7)
vwire(147.9, Y2 + 7.9, SWY + 12.7)
via1(147.9, Y2 + 7.9)
...
via2(118.4, -34.5)
rect("m3", 118.2, -34.7, 192.0, -34.3)                         # trk shelf
via2(191.8, -34.5)
vwire(191.8, TRK2, -34.5)
via2(191.8, TRK2)
rect("m3", 149.2, TRK2 - 0.2, 192.0, TRK2 + 0.2)               # row-2 trk band: inv..TG2
via2(119.2, TRKB_Y)
vwire(119.2, -35.4, TRKB_Y)
via2(119.2, -35.4)
rect("m3", 119.0, -35.6, 192.8, -35.2)                         # trkb shelf
via2(192.6, -35.4)
vwire(192.6, TRKB2, -35.4)
via2(192.6, TRKB2)
rect("m3", 99.0, TRKB2 - 0.2, 192.8, TRKB2 + 0.2)              # row-2 trkb band: bstrap ckb + inv..TG2
...
rect("m3", 163.0, VIN2 - 0.2, 190.6, VIN2 + 0.2)               # row-2 vcm band: TG vin .. TG2 vin
via2(163.2, VIN2)
vwire(163.2, VIN2, -9.82)                                       # riser to the -9.82 vcm bridge
via2(163.2, -9.82)
...
via2(169.6, -9.82)
rect("m3", 160.0, -10.02, 169.8, -9.62)
...
layout.write("oa_sar8_acore8.gds")
b = top.dbbox()
print(f"acore8: {b.width():.0f} x {b.height():.0f} um = {b.width()*b.height()/1e6:.3f} mm2")
