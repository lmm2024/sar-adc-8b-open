...
layout.dbu = 0.001
top = layout.create_cell("oa_sar8_core13")
...
# outn: bar (168.9..250 @ 110.2) -> M4 east across the replica array
via3(249.5, 110.2)
x1, y1 = snap(249.5 - 0.095), snap(110.2 - 0.095)
rect("v4", x1, y1, x1 + 0.19, y1 + 0.19)
rect("m5", 249.3, 110.0, XE, 110.4)                            # M5: crosses the topr M4 riser (299.3) on another layer
top.shapes(L["m5p"]).insert(pya.DBox(XE - 1.5, 110.0, XE, 110.4))
top.shapes(L["m5l"]).insert(pya.DText("outn", pya.DTrans(pya.DVector(XE - 0.75, 110.2))))

layout.write("oa_sar8_core13.gds")
b = top.dbbox()
print(f"core13: {b.width():.0f} x {b.height():.0f} um = {b.width()*b.height()/1e6:.3f} mm2")
