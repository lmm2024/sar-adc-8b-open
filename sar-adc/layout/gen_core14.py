#!/usr/bin/env python3
"""OA-SAR8 core14 -- closed-loop fixed (harden_v8 macro): comparator clock from the
...
layout.dbu = 0.001
top = layout.create_cell("oa_sar8_core14")

layout.read("oa_sar8_acore11.gds")
layout.read("/foss/designs/sar-adc/logic/final/gds/sar_ctrl.gds")
ac = layout.cell("oa_sar8_acore11")
...
# vcm: -9.82 band (133..169.8) -> M4 riser at 161.6 down to -45.5 -> M3 east to the boundary
rect("m3", 160.0, -10.02, 161.8, -9.62)
via3(161.6, -9.82)
v4wire(161.6, YS, -9.82)
top.shapes(L["m4p"]).insert(pya.DBox(161.4, YS, 161.8, YS + 1.5))
top.shapes(L["m4l"]).insert(pya.DText("vcm", pya.DTrans(pya.DVector(161.6, YS + 0.75))))
...
layout.write("oa_sar8_core14.gds")
b = top.dbbox()
print(f"core14: {b.width():.0f} x {b.height():.0f} um = {b.width()*b.height()/1e6:.3f} mm2")
