...
layout.dbu = 0.001
top = layout.create_cell("oa_sar8_core9")

layout.read("oa_sar8_acore8.gds")
layout.read("/foss/designs/sar-adc/logic/final/gds/sar_ctrl.gds")
ac = layout.cell("oa_sar8_acore8")
...
for k in range(8):
    ...
    vwire(xd, -33.0, 0.4)                                # M2 drop into the pin metal
    via2(xd, -33.0)
    via3(xd, -33.0)
    v4wire(xd, yt, -33.0)
    via3(xd, yt)
...
for name, lay_, x1, y1, x2, y2 in (("vin", "m3", 96.5, -43.1, 97.1, -42.7),
...
layout.write("oa_sar8_core9.gds")
b = top.dbbox()
print(f"core9: {b.width():.0f} x {b.height():.0f} um = {b.width()*b.height()/1e6:.3f} mm2")
