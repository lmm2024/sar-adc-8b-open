...
layout.dbu = 0.001
top = layout.create_cell("sw_bitcell11")
...
rect("m1", XL, -3.2, XR, -2.6)      # VSSB: substrate/guard rail (global VSS), separate from the gated VSS
for d in (nd, nt):
    g = d.t("grd")
    if d is nd:
        xl_ = g.right - 0.45                                       # nd: use the EAST guard leg (gap between nd and nt)
        rect("m1", xl_, -1.0, xl_ + 0.55, g.bottom + 0.12)
        xh = xl_ + 0.4
    else:
        xl_ = g.left + 0.15
        rect("m1", xl_ - 0.35, -1.0, xl_ + 0.3, g.bottom + 0.12)
        xh = xl_ - 0.15
    via1(xh, -0.8)
    vwire(xh, -0.8, -2.9)                                          # M2 hop over the VSS rail
    via1(xh, -2.9)
for d in (pd, pt):
    g = d.t("grd")
...
for d in (nd, pd):
    ...
    vwire(gx, 3.9, gy)
    via2(gx, 3.9)
# gb (boosted gate bus, local 6.92..7.32): nt gate rises past the trk band
hbus(6.92, -1.0, 12.2)
gx, gy = gate_tap(nt)
vwire(gx, gy, 7.12)
via2(gx, 7.12, ph=0.19)
gx, gy = gate_tap(pt)
vwire(gx, 3.1, gy)
via2(gx, 3.1)

# pins
pins = (("bot", 5.3), ("vin", 4.5), ("ctl", 3.7), ("trkb", 2.9), ("gb", 6.92))
for name, y in pins:
...
top.shapes(L["m1p"]).insert(pya.DBox(XL, -3.1, XL + 0.8, -2.7))
top.shapes(L["m1l"]).insert(pya.DText("VSSB", pya.DTrans(pya.DVector(XL + 0.4, -2.9))))
...
layout.write("sw_bitcell11.gds")
print("wrote sw_bitcell11.gds bbox:", top.dbbox())

with open("sw_bitcell11.cdl", "w") as f:
    f.write("""* bottom-plate driver + boosted-gate vin switch, gated VSS + separate substrate rail
.subckt sw_bitcell11 bot vin ctl trkb gb VDD VSS VSSB
MND bot ctl VSS VSSB sg13_lv_nmos w=2.0u l=0.13u rfmode=1
MPD bot ctl VDD VDD sg13_lv_pmos w=4.0u l=0.13u rfmode=1
MNT bot gb vin VSSB sg13_lv_nmos w=2.0u l=0.13u rfmode=1
MPT bot trkb vin VDD sg13_lv_pmos w=4.0u l=0.13u rfmode=1
.ends
""")
print("wrote sw_bitcell11.cdl")
