...
set filetype=ascii
tran 0.05n 500n uic
meas tran iavg AVG i(VDD) from=160n to=490n
echo "TOTAL_CORE_IAVG_A = $&iavg"
wrdata /foss/designs/sar-adc/power/mixed2_out.csv v(done) v(result7) v(result6) v(result5) v(result4) v(result3) v(result2) v(result1) v(result0) v(trk) v(vinp) v(vinn) i(VDD) v(cmpq) v(outp) v(outn) v(clk_cmp)
quit
.endc
.end
"""
open("/foss/designs/sar-adc/power/tb_mixed2.spice", "w").write(tb)
print("tb_mixed2.spice written", len(tb) // 1024, "KB")
