...
wrdata /foss/designs/sar-adc/power/mixed4lf{TAG}_out.csv v(done) v(result7) v(result6) v(result5) v(result4) v(result3) v(result2) v(result1) v(result0) v(trk) v(hold) v(vinp) v(vinn) i(VDD) v(outp) v(outn) v(clk_cmp)
quit
.endc
.end
"""
open(f"/foss/designs/sar-adc/power/tb_mixed4lf{TAG}.spice", "w").write(tb)
print(f"tb_mixed4lf{TAG}.spice written", len(tb) // 1024, "KB")
