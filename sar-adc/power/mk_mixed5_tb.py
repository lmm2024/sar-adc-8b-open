...
ACORE = "/foss/designs/sar-adc/power/acore12_sch.spice" if "sch" in sys.argv[4:] else ([a[4:] for a in sys.argv[4:] if a.startswith("pex=")] or ["/foss/designs/sar-adc/layout/pex_a12/oa_sar8_acore12.pex.spice"])[0]
...
if INJ:
    ...
    lines = ac.split("\n")
    j = next(i for i, ln in enumerate(lines) if ln.startswith(".subckt oa_sar8_acore12"))
...
if MERGE:
    ...
    j = next(i for i, ln in enumerate(keep) if ln.startswith(".subckt oa_sar8_acore12"))
...
wrdata /foss/designs/sar-adc/power/mixed5{TAG}_out.csv v(done) v(result7) v(result6) v(result5) v(result4) v(result3) v(result2) v(result1) v(result0) v(trk) v(hold) v(vinp) v(vinn) i(VDD) v(outp) v(outn) v(clk_cmp) v(xacore.sw_tg_0.bot) v(xacore.cdac_array_1.topp)
quit
.endc
.end
"""
open(f"/foss/designs/sar-adc/power/tb_mixed5{TAG}.spice", "w").write(tb)
print(f"tb_mixed5{TAG}.spice written", len(tb) // 1024, "KB")
