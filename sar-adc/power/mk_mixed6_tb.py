...
wrdata /foss/designs/sar-adc/power/mixed6{TAG}_out.csv v(done) v(result7) v(result6) v(result5) v(result4) v(result3) v(result2) v(result1) v(result0) v(trk) v(hold) v(vinp) v(vinn) i(VDD) v(outp) v(outn) v(clk_cmp) v(xacore.sw_tg_0.bot) v(xacore.cdac_array_1.topp)
quit
.endc
.end
"""
open(f"/foss/designs/sar-adc/power/tb_mixed6{TAG}.spice", "w").write(tb)
print(f"tb_mixed6{TAG}.spice written", len(tb) // 1024, "KB")
