...
def lg_conn(p):
    m = {f"dac_code[{k}]": f"ctl{k}" for k in range(8)}
    m.update({f"dac_code_n[{k}]": f"ctl{k}n" for k in range(8)})
    m.update({"cmp": "outp", "sample": "trk", "clk": "clk_cmp", "VDD": "vdd", "VSS": "0",
...
VDD vdd 0 {{vdd}}
VCM vcm 0 0.75
Vinp vinp 0 dc 0.75 sin(0.75 0.5 1.0meg 0 0)
Vinn vinn 0 dc 0.75 sin(0.75 -0.5 1.0meg 0 0)
Vclk clk_cmp 0 pulse(0 {{vdd}} 0 0.1n 0.1n 4.9n 10n)
Vrst rst_n 0 pwl(0 0 30n 0 30.5n {{vdd}})
* start pulses every 100 ns (10 MS/s), each 10 ns wide, first at 45 ns
Vst start 0 pulse(0 {{vdd}} 45n 0.1n 0.1n 10n 100n)
...
set filetype=ascii
tran 0.05n 560n uic
meas tran iavg AVG i(VDD) from=160n to=560n
echo "TOTAL_CORE_IAVG_A = $&iavg"
wrdata /foss/designs/sar-adc/power/mixed_out.csv v(done) v(result7) v(result6) v(result5) v(result4) v(result3) v(result2) v(result1) v(result0) v(trk) v(vinp) v(vinn) i(VDD)
quit
.endc
.end
"""
open("/foss/designs/sar-adc/power/tb_mixed.spice", "w").write(tb)
print("tb_mixed.spice written", len(tb) // 1024, "KB")
